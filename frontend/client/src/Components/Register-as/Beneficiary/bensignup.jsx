...
export default function Bensignup() {
  ...
  const register=()=>{
    const beneficiary = {name,username, email, mobile, password, confirmPassword} ;
    if(name&&username&&email&&mobile&&password){
        if(password===confirmPassword){
            axios.post("http://localhost:9002/bensignup",beneficiary)
            .then(res=>
  ...
  return (
    <div className='body'>
        <div className="cont">
        <div className="title">Registration <label>as beneficiary</label></div>
        <div className="content">
            <form onSubmit={handleSubmit}>
                ...
                <div align= "center"><label>Already register? <Link to="/benlog" >login here</Link></label></div>
            </form>
        </div>
    </div>
    <ToastContainer/>
